...
async function endApplication(appName) {
  if (appName === 'chrome') {
    try {
      ...
      await keyboard.pressKey(Key.LeftSuper, Key.Q); // Key.LeftSuper represents the Command key
      await new Promise(resolve => setTimeout(resolve, 2000)); // Hold for 2 seconds
      await keyboard.releaseKey(Key.LeftSuper, Key.Q);
    } catch (error) {
      console.error('Error in closing Chrome: ', error);
      throw error;
    }
  }
}
